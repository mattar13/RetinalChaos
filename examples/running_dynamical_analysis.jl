...
#%% Run the analysis without sodium influence
...
prob = BK.BifurcationProblem(
    ...
    J=J,
    recordFromSolution=(x, p) -> (V=x[1], N=x[2])
)
...
for sol in br_pocoll.sol
    ...
    plot!(periodic_orbits, traj[1, :], traj[2, :], xlabel="E", ylabel="x", label="")
end
periodic_orbits

#%% Determine the sodium conductance: Running an ensemble problems
#Step 3: set up the problem making sure enable jacobian is true
reload_parameters()
parameters[I_app] = -50.0 #In order for convergence to occur, I_app needs to be set low
parameters[g_ACh] = 0.0
parameters[g_GABA] = 0.0
parameters[g_TREK] = 0.0 #Remove the sAHP
...
# continuation options (Theses need to be tuned)
opts_br = BK.ContinuationPar(
    pMin=-150.0, pMax=100.0, #When plotting the bifurction for sodium, increase to intercepts to -150
    ds=0.04, dsmax=1.0,# this is to detect bifurcation points precisely with bisection
    detectBifurcation=3, # Optional: bisection options for locating bifurcations
    nInversion=8, maxBisectionSteps=50,
    maxSteps=500,
...
#Determine where the periodic orbits exist
optn_po = BK.NewtonPar(verbose=true, tol=1e-8, maxIter=10)
opts_po_cont = BK.ContinuationPar(
    dsmin=0.04, ds=0.1, dsmax=2.0,
    pMin=-50.0, pMax=50.0,
    maxSteps=110,
