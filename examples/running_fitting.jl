...
ePhys.truncate_data!(data, t_begin=1.0, t_end=10e3)
data - 10.0
...
SDEProblem(SDEModel, u0, (tmin, 300e3), parameters)
warmup = solve(probSDE, callback=cbs, EM(), dt=dt, save_everystep=false)
...
@time sol = solve(probSDE, callback=cbs, EM(), dt=dt); #So far the best method is SOSRI
