...
#%% This diffusion figure is very confusing
bp_model = Network(nx, ny, μ = 0.15)
#we want to make a network in the range of burst-2000 

#%%

time_range = collect(sol.t[]:dt:xlims[2])
lattice_c = zeros(nx, ny, length(time_range))
for (idx, t) in sol.t
    if idx == 1
        lattice_c[c1x, c1y, idx] = sol(t, idxs = 6)
    else
...
anim = @animate for frame in xlims[1]:d_frame:xlims[end]-1
    ...
    frame_i = lattice_c[:,:, round(Int64, frame)]
...
for (idx, frame) in enumerate(frame_stops)
    ...
    if idx == length(frame_stops)
        heatmap!(fig2_B[idx], lattice_c[:,:,frame_idx], c = :thermal, clims = (0.0, upper_lim), 
            aspect_ratio = :equal, grid = false, colorbar_title = "Eₜ (μM)",
