...
# ╔═╡ fa4f6364-0c66-4bbf-a602-a63356b524a0
...
# ╔═╡ b2e3f6ef-5f9d-41b8-ab30-8056fef46ebd
plot(sol, vars = 1)

# ╔═╡ f57aa1f7-41b4-435f-b6fc-24815a655b93
begin 
	#Run analysis 
	v_thresh = [calculate_threshold(sol)[1]]
	timestamps = get_timestamps(sol, v_thresh)
	burst_timestamps, spd = max_interval_algorithim(timestamps)
end
...
# ╔═╡ e9dd2cad-4fd2-45e6-900f-d3afd44af15b
plot(sol, vars = (1,2))
...
begin
	...
	fig3_C = vspan([bif_val[end], c1_lims[end]], label = "",
			c = :green, alpha = 0.5, grid = false
		) 
	vspan!(fig3_C, [c1_lims[1], bif_val[end]], label = "", 
		c = :red, alpha = 0.5, dpi = 300
	)
	noise_vals = sol(sol.t, idxs = 7)
	hfit = fit(Histogram, noise_vals, LinRange(c1_lims[1], c1_lims[2], 1000))
	weights = hfit.weights/maximum(hfit.weights)
	edges = collect(hfit.edges[1])[1:length(weights)]
	plot!(fig3_C, edges, weights, dpi = 300, label = "", c = :blue,
		xlabel = "Noisy current (pA)", ylabel = "Probability of Occurrance (PDF)", 
		xlims = c1_lims, fillcolor = :blue, fillrange = [0.0], alpha = 0.75
	)	
end
...
begin
	...
	#Place 3 annotations
	annotate!(fig3_labels[1], [0.5], [0.99], "A", font("Sans",24))
	annotate!(fig3_labels[2], [0.5], [0.99], "B", font("Sans",24))
	annotate!(fig3_labels[3], [0.5], [0.99], "C", font("Sans",24))
	fig3_graphs = plot(fig3_A, fig3_B, fig3_C, 
		layout = grid(3,1), size  = (1000,1000)
	)
	fig3 = plot(fig3_labels, fig3_graphs, 
...
begin
	file_root = "F:\\Projects\\2021_Modelling_Paper\\Figures\\"
	savefig(fig3, "$(file_root)Fig2_Spontneous_Activity.png")
end

# ╔═╡ 00000000-0000-0000-0000-000000000001
PLUTO_PROJECT_TOML_CONTENTS = """
[deps]
RetinalChaos = "3f100aef-eb91-46ae-997e-7902479fb568"
...
[[ConfigEnv]]
git-tree-sha1 = "1891cfd27ae8256e4e69574c93dc0d6eb839903e"
...
[[JSON2]]
deps = ["Dates", "Test"]
git-tree-sha1 = "1914b73df49ad202a51a81f08934c878bdda3ae7"
...
git-tree-sha1 = "bf6e16e95a27e354c999d96d5d2e339c949aa866"
...
[[RetinalChaos]]
deps = ["BSON", "CUDA", "ColorSchemes", "Colors", "ConfigEnv", "Dates", "DiffEqBase", "DiffEqCallbacks", "DifferentialEquations", "Distributions", "ForwardDiff", "ImageSegmentation", "Images", "JLD2", "JSON2", "LaTeXStrings", "LinearAlgebra", "LsqFit", "Measures", "ModelingToolkit", "NLsolve", "OrdinaryDiffEq", "Plots", "ProgressMeter", "PyCall", "RandomNumbers", "ResettableStacks", "Revise", "Statistics", "StatsBase", "StatsPlots", "StochasticDiffEq", "Telegram", "TerminalLoggers"]
git-tree-sha1 = "92bb801f3efc55e671624371070aaafd71dcd5fc"
repo-rev = "master"
repo-url = "https://github.com/mattar13/RetinalChaos.git"
uuid = "3f100aef-eb91-46ae-997e-7902479fb568"
version = "0.1.0"
...
git-tree-sha1 = "bf199d948b4f629b1ed3745c071e431bdd4f3c0f"
...
[[Telegram]]
deps = ["HTTP", "JSON3"]
git-tree-sha1 = "3febbb0ae6b38ba55ac800e1a592d7f1b7bc3668"
...
[[Tokenize]]
git-tree-sha1 = "fd73da0afb003ce6dc539f029681d5c24ebbc121"
...
git-tree-sha1 = "777a3ca87b8d28eaa42bc58b125ab6ba08fb417e"
