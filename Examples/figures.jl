...
tspan = (0.0, 60e3)
dt = 0.1 #This is only for sampling datapoints in the solution
#Create the problem
...
fig1_A = plot(fig1_Aa, fig1_Ab, 
    ...
    layout = grid(1,2, widths = [0.7, 0.3]), grid = false
);
...
fig1_C = plot(fig1_Ca, fig1_Cb, fig1_Cc, 
    ...
    layout = grid(1,3, widths = [0.6, 0.2, 0.2]), grid = false
)
...
output, _, _, _ = max_interval_algorithim(spike_array; dt = 1.0);
burst2 = output[2]
println(burst2)
...
fig2_B = plot(xlims = (0, nx), ylims = (0, ny), layout = grid(1, length(frame_stops)), gridlines = false, size = (1000, 250), 
    margin = 1mm
)
