...
function animate_solution(data, loc; 
    ...
    animation_filename = "regular_animation", 
    cmin = -85.0, dc = 5.0, cmax = -5.0,
    cticks = [-80, -40.0, 0.0],
    extension = ".mp4"
)
    if verbose
    ...
    if isfile("$(loc)\\$(animation_filename)$(extension)")
        rm("$(loc)\\$(animation_filename)$(extension)")
    end
    data_arr = reshape(data["DataArray"], (xmax, ymax, size(data["DataArray"], 2)))
    #%%
    figsize = (4, 6)
    fig, ax = plt.subplots(figsize=figsize)
    im1 = ax.contourf(data_arr[:, :, 1], levels=cmin:dc:cmax, cmap="viridis", extend="both")
    ...
    function update(idx)
        ...
        im1 = ax.contourf(frame, levels=cmin:dc:cmax, cmap="viridis")
    ...
    #plt.show()
    animation.save("$(loc)\\$(animation_filename)$(extension)", fps=5*(1000.0/animate_dt), dpi=100, writer="imagemagick")
end
