...
function (PDE::Network{T, :Lansdell} where T)(dU, U, p, t)
    ...
    (I_app, E_Ca, E_K, E_leak, E_ACh, V1, V2, V3, V4, g_Ca, g_K, g_leak, g_noise, g_ACh, δ, C_m, τr, τs, τACh, γ, α, β, k, V0, D) = p
    ...
    @. dv = (
            ...
            + fI(g_noise, abs(W), v, E_Ca)
            + I_app
        )/C_m

    @. dr = (1/τr) * (Λ(v, V3, V4)*(N_INF(v, V3, V4) - r) + α*s*(1-r))
    @. ds = -s/τs + γ*Φ(v, k, V0) 
    mul!(PDE.MyA, PDE.My, a)
    mul!(PDE.AMx, a, PDE.Mx)
    @. PDE.DA = D*(PDE.MyA + PDE.AMx)
    @. da = -a/τACh + PDE.DA + β*Φ(v, k, V0) 
    @. dW = -W
    nothing
end 
