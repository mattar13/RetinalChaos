...
function T_ODE(dU::AbstractArray{T2}, U::AbstractArray{T2}, p::AbstractArray{T2}, t::T; release_mode=:constant) where {T,T2}
     #Extract all of the parameters first
...
#This includes the GABA term
function T_SDE(dU::AbstractArray{T2}, U::AbstractArray{T2}, p::AbstractArray{T2}, t::T) where {T,T2}
     #Extract all of the parameters first
     (g_leak, E_leak, g_Ca, V1, V2, E_Ca, g_K, E_K, g_TREK, g_ACh, k_ACh, E_ACh, g_GABA, k_GABA, E_GABA, I_app, C_m, V3, V4, τn, C_0, λ, δ, τc, α, τa, β, τb, ρe, ρi, τACh, τGABA, ke, ki, V0e, V0i, De, Di, τw, σ) = p
     v = view(U, 1)
     n = view(U, 2)
...
#These next two functions vary the Neurotransmitter release with voltage
function T_ODE_NT_Release(dU::AbstractArray{T2}, U::AbstractArray{T2}, p::AbstractArray{T}, t::T) where {T<:Real,T2}
     #Extract all of the parameters first
     (g_leak, E_leak, g_Ca, V1, V2, E_Ca, g_K, E_K, g_TREK, g_ACh, k_ACh, E_ACh, g_GABA, k_GABA, E_GABA, I_app, C_m, V3, V4, τn, C_0, λ, δ, τc, α, τa, β, τb, ρe, ρi, τACh, τGABA, ke, ki, V0e, V0i, De, Di, τw, σ) = p
     v = view(U, 1)
     n = view(U, 2)
     ...
     @. da = (α * c^4 * (1 - a) - a) / τa
     @. db = (β * a^4 * (1 - b) - b) / τb
     @. de = (ρe * Φ(v, ke, V0e) - e) / τACh #These terms are no longer constant
     @. di = (ρi * Φ(v, ki, V0i) - i) / τGABA #These terms are no longer constant
     @. dW = -W / τw
     #nothing
     dU
end
...
#These next two functions vary the Neurotransmitter release with voltage
function T_SDE_NT_Release(dU::AbstractArray{T2}, U::AbstractArray{T2}, p::AbstractArray{T}, t::T) where {T<:Real,T2}
     #Extract all of the parameters first
     (g_leak, E_leak, g_Ca, V1, V2, E_Ca, g_K, E_K, g_TREK, g_ACh, k_ACh, E_ACh, g_GABA, k_GABA, E_GABA, I_app, C_m, V3, V4, τn, C_0, λ, δ, τc, α, τa, β, τb, ρe, ρi, τACh, τGABA, ke, ki, V0e, V0i, De, Di, τw, σ) = p
     v = view(U, 1)
     n = view(U, 2)
     ...
     @. da = (α * c^4 * (1 - a) - a) / τa
     @. db = (β * a^4 * (1 - b) - b) / τb
     @. de = (ρe * Φ(v, ke, V0e) - e) / τACh #Release is voltage dependant
     @. di = (ρi * Φ(v, ki, V0i) - i) / τGABA #Release is voltage dependant
     @. dW = -W / τw
     #nothing
     dU
end